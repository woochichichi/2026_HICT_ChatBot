...
def s13_numbers():
    ...
    # 정직성: 아래 수치는 30문항 기준. '100문항'은 재측정 예정(계획)일 뿐이며 측정치가 아님.
    ...
    for big, sub in kpis:
        ...
        shape_text(c, [
            ...
            anchor=MSO_ANCHOR.MIDDLE, space=3)
        x = Emu(int(x + w + gap))
    put_text(s, Inches(0.62), Inches(5.45), Inches(12.1), Inches(0.5),
             [("신뢰도를 더 다지기 위해 평가 문항을 30 → 100문항으로 늘려 재측정 예정",
               11.5, False, TAUPE, PP_ALIGN.CENTER)])
    takeaway(s, "검색은 이미 최상위, 답변은 목표를 향해")
