...
반입 파일: `scripts/wiki_fetch.ps1`, `scripts/wiki_check.ps1`, `scripts/wiki_fetch.config.txt`

**먼저 어떤 인증이 되는지 확인** (쿠키 없이 Windows 로그인부터 시도):

```
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\wiki_check.ps1
```

→ 마지막 줄 RESULT 를 보고 아래 중 하나로 수집:

```
# (A) "Windows auth WORKS" 인 경우 — 쿠키 불필요 (실측: 사내 위키는 이 방식으로 통과)
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\wiki_fetch.ps1 -WindowsAuth

# (B) 쿠키가 필요한 경우 — 쿠키를 "알아서 파싱"
#   1) F12 → Network → 위키 요청 우클릭 → Copy → "Copy as cURL"
#   2) 그 내용을 scripts\wiki_auth.txt 에 통째로 붙여넣고 저장 (cURL/헤더/쿠키 어느 형식이든 OK)
#   3) 아래 실행 (스크립트가 cURL 안에서 cookie를 자동 추출)
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\wiki_fetch.ps1

# 매일 변경분만 (최근 업데이트 화면)
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\wiki_fetch.ps1 -WindowsAuth -Incremental
```

→ `pages ... to fetch` 로그 후 `page_<id>.html` 들이 `scripts\wiki_html\` 에 저장됨.

> 사내 인증서 오류 시: 회사 CA가 Windows 신뢰저장소에 있으면 자동 통과됨.
> 그래도 막히면 `-SkipCertCheck` (단, 신뢰 가능한 내부망에서만).
>
> **쿠키 자동 추출**: `wiki_auth.txt` 에는 "Copy as cURL" 전체를 그대로 넣어도 되고,
> "Copy request headers" 블록을 넣어도 되고, `JSESSIONID=...; mywork=...` 쿠키 한 줄만
> 넣어도 됩니다. 스크립트가 셋 다 인식해 cookie 값만 뽑아냅니다.
