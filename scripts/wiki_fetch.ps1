...
[CmdletBinding()]
param(
    [string]$ConfigPath = "$PSScriptRoot\wiki_fetch.config.txt",
    [string]$AuthValue,
    [switch]$Incremental,
...
$baseUrl   = (Get-Cfg $cfg 'BASE_URL'   '').TrimEnd('/')
$spaceKey  =  Get-Cfg $cfg 'SPACE_KEY'  ''
...
$outDir    =  Get-Cfg $cfg 'OUT_DIR'    "$PSScriptRoot\wiki_html"
...
if ($baseUrl -eq '' -or $spaceKey -eq '') {
    throw 'BASE_URL and SPACE_KEY are required in the config file'
}

# --- auth header value: param > file > prompt ---
if (-not $AuthValue) {
    $authFile = Join-Path $PSScriptRoot 'wiki_auth.txt'
